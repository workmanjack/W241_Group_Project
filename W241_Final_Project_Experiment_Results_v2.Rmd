...
Three rounds of survey requests were issued on *Mechanical Turk*:

 1. For the pilot study and asked for 50 responses. All 50 were given. 
 2. For the experiment, required a Masters qualification, and asked for 300 responses. Roughly 40 were returned. 
 3. For the experiment, did not require a Masters qualification, and asked for 300 responses. All 300 were provided.
...
Unfortunately, about have of 519 responses had to be flagged as invalid. Our criteria for an invalid response is as follows:
...
Qualtrics was allocation surverys at random order to the above three groups. As we can see from `Table 2` there was some slight inbalance as `Treat: Low` received a bit lower than fair share of responses. We'll assume it's due to random noise.
...
As indicated in `Figure 2` we dropped `r nrows_raw-excl_dups_unfinished` responses because they either did not finish the entire survey or did not answer the main question. `Table 3` checks distribution of these responses across treatment groups. The only issue that stands out is that unproportionally low number of unfinished surveys were in `Treat: Low` group, but it's consistent with the fact that Qualtrics allocation was not even. 
...
The last check is to see how valid responses were distributed across source groups (`Table 5`). Interestingly enough, `Treat: Low` group has more responses in the `Friends and Family` group compared to `Treat: High` and `Control`. We don't have a strong hypothesis to explain this inconsistency and we'll assume that it's random.
...
Responders had to rate the story on a scale from 1 to 6. We decided to use even scale to make responders pick a side instead of providing a neutral response. Based on `Figure 3a` for `Control` group, average rating without any treatment was above 3.5 (expected average). This indicates that the story was slightly better than average. 

`Treat High` group had the highest proportion of 5-star ratings, but it was not enough to increase group's average rating compared to the `Control` group. `Treat Low` group, on the other hand, had lower average rating compared to two other groups.
...
The average control rating `r round(avg_c, 2)`. The average low rating was `r round(avg_t1,2)`. The average high rating was `r round(avg_t2,2)`. As you can see, the largest effect (in absolute terms) was caused by the low rating treatment. Interestingly, the average high rating is slightly lower than the average control. These results suggest that users are significantly more influenced by lower reviews than high ones.
...
Since we excluded a lot of responses due to serious validity issues, we want to check if including them changes our results. `Table 8` compared outputs of 3 models:
...
However, if we check `Table 9` below, the majority of responders that failed reading comprehension were from `MTurk Regulars` group. Therefore, we can suggest two hypothesis for the above results:
 ...
 1. `MTurk Regulars` did not read the story carefully and, hence, did not have strong opinion on it. Then they picked the score that was closer to expected average rating
 2. `MTurk Regulars` did not read the story at all (potentially bots) and picked the score that was closer to expected average rating

Regardless of the explanations, the results are still consistent: the impact of Low rating is statistically significant, while High ratings doesn't make people change their minds.
...
There results suggest that:
 ...
 - In none of the three groups `High` rating impacted people's opinions about the story (as compared to the `Control` group rating)
 - In  `MTurk` groups `Low` rating was 0.8 to 1 lower than average `Control` group score. However, effect estimate for the `Masters` group is not statistically significant mostly due to low sample size
 - For the `F&F` group, neither of the treatment made a difference. Their average ratings was statstically indistinguishable across all treatment and control groups
...
## 5.1 Potential Experimental Pitfalls
